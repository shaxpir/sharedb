...
function Connection(socket, options) {
  ...
  if (options && options.durableStore) {
    // Create the DurableStore instance.
    var connection = this;
    this.durableStore = new DurableStore(options.durableStore);
    this.durableStore.on('ready', function() {
      ...
      if (options.durableStore.onReadyCallback) {
        options.durableStore.onReadyCallback();
      }
    });
