...
function Connection(socket, options) {
  ...
  if (options && options.durableStore) {
    // Create the DurableStore instance with the provided storage engine.
    var connection = this;
    var durableStoreOptions = options.durableStore;
    var storage = durableStoreOptions.storage;
    if (!storage) {
      throw new Error('durableStore.storage is required when using durableStore');
    }
    
    // Create the DurableStore with the storage instance and remaining options
    var storeOptions = {
      maxBatchSize: durableStoreOptions.maxBatchSize,
      extVersionDecoder: durableStoreOptions.extVersionDecoder,
      opErrorCallback: durableStoreOptions.opErrorCallback,
      debug: durableStoreOptions.debug
    };
    
    this.durableStore = new DurableStore(storage, storeOptions);
    this.durableStore.on('ready', function() {
      connection._onDurableStoreReady(socket);
      // If the application supplied an "on-ready" callback, call it now.
      if (durableStoreOptions.onReadyCallback) {
        durableStoreOptions.onReadyCallback();
      }
    });
    this.durableStore.initialize();
  } else {
...
Connection.prototype._onDurableStoreReady = function(socket) {
  var connection = this;
  var durableStore = this.durableStore;
  // Set the connection state and bind to the socket.
  connection.state = connectionState(socket);
  connection.bindToSocket(socket);
  // Reload all docs from the DurableStore with pendingOps
  // that need to be synchronized with the server.
  durableStore.loadEachPendingDocAndThen(function(error, pendingDoc) {
    if (error || !pendingDoc) return;
    var collection = pendingDoc.collection;
    var id = pendingDoc.id;
    if (!(collection in connection.collections)) {
      connection.collections[collection] = Object.create(null);
    }
    var doc = new Doc(connection, durableStore, collection, id);
    connection.collections[collection][id] = doc;
    durableStore.restoreDocFromDurableRecord(doc);
  });
};

/**
 * Returns true if there is an offline store, and if that offline store contains
 * any docs with offline state waiting for the socket to be re-opened, so that
 * their state can be syncrhonized with the server.
 */
Connection.prototype.hasPendingOfflineDocs = function() {
  return this.durableStore && this.durableStore.hasPendingDocs();
};

/**
 * Returns true if there is an entry in the inventory for the given collection and id,
 * indicating that the doc has been persisted in the DurableStore.
 */
Connection.prototype.isDocInInventory = function(collection, id, minVersion) {
  return this.durableStore && this.durableStore.isDocInInventory(collection, id, minVersion);
};

/**
* If there is a DurableStore, this callback will be executed once (with 'collection'
* and 'id' args) for each doc that exists in that store and has offline pending-ops
* waiting to be synchronized to the server.
*/
Connection.prototype.forEachPendingDocCollectionId = function(callback) {
  if (this.durableStore) {
    this.durableStore.forEachPendingDocCollectionId(callback);
  }
}
...
Connection.prototype.get = function(collection, id) {
  ...
  var doc = docs[id];
  if (!doc) {
    doc = docs[id] = new Doc(this, this.durableStore, collection, id);
    this.emit('doc', doc);
  }
...
/**
 * Get multiple documents by ID in a single batch operation.
 * This method leverages caching to avoid unnecessary storage lookups.
 * 
 * @param {string} collection - Collection name
 * @param {string[]} ids - Array of document IDs to retrieve
 * @param {function} callback - Callback function (error, docs)
 * @return {void}
 */
Connection.prototype.getBulk = function(collection, ids, callback) {
  ...
  // First pass: check cache and create/retrieve Doc objects
  for (var i = 0; i < ids.length; i++) {
    ...
    if (doc) {
      ...
      uncachedIds.push(id);
      // Create the Doc object now so it's in cache for future calls
  ...
  // Need to fetch uncached documents from storage
  if (this.durableStore) {
    this.durableStore.retrieveDocumentsBulk(collection, uncachedIds, function(error, docDatas) {
      if (error) return callback(error);
      
      // Populate the Doc objects with data from storage
      if (docDatas && docDatas.length > 0) {
        for (var k = 0; k < docDatas.length; k++) {
          var docData = docDatas[k];
          var doc = cachedDocs[docData.id];
          if (doc && docData.data !== undefined) {
            // Populate the document with retrieved data
            doc._handleFetch(null, docData);
          }
        }
      }
    ...
    // No durable store - return empty docs (consistent with single get() behavior)
    for (var m = 0; m < ids.length; m++) {
      results.push(cachedDocs[ids[m]]);
    }
    callback(null, results);
  }
};
...
/**
 * Control automatic flushing of writes to the DurableStore.
 * When auto-flush is disabled, documents are queued but not automatically written to storage.
 * 
 * @param {boolean} enabled - Whether to enable automatic flushing
 */
Connection.prototype.setAutoFlush = function(enabled) {
  if (this.durableStore) {
    this.durableStore.setAutoBatchEnabled(enabled);
  }
};
 ...
 * 
 * @return {boolean} - True if auto-flush is enabled
 */
Connection.prototype.isAutoFlush = function() {
  return this.durableStore ? this.durableStore.isAutoBatchEnabled() : true;
};

/**
 * Add a document to the write queue. If auto-flush is enabled, this triggers automatic flushing.
 * If auto-flush is disabled, documents are queued without flushing.
 * 
 * @param {Doc} doc - ShareDB document to write
 * @param {function} callback - Optional callback function
 */
Connection.prototype.putDoc = function(doc, callback) {
  if (this.durableStore) {
    this.durableStore.putDoc(doc, callback);
  } else if (callback) {
    callback(new Error('No DurableStore available'));
  }
};
 ...
 * @param {function} callback - Optional callback function
 */
Connection.prototype.putDocs = function(docs, callback) {
  if (!this.durableStore) {
    if (callback) callback(new Error('No DurableStore available'));
    return;
  }
  ...
  for (var i = 0; i < docs.length; i++) {
    this.durableStore.putDoc(docs[i], function(error) {
      if (hasError) return;
 ...
 * This is a convenience method that temporarily disables auto-flush,
 * adds all documents, flushes, then restores the original setting.
 * 
 * @param {Doc[]} docs - Array of ShareDB documents to write
 * @param {function} callback - Optional callback function  
 */
Connection.prototype.putDocsBulk = function(docs, callback) {
  if (!this.durableStore) {
    if (callback) callback(new Error('No DurableStore available'));
    return;
  }
  
  this.durableStore.putDocsBulk(docs, callback);
};

/**
 * Force flush any pending writes in the DurableStore queue.
 * 
 * @param {function} callback - Optional callback function
 */
Connection.prototype.flushWrites = function(callback) {
  if (this.durableStore) {
    this.durableStore.flush(callback);
  } else if (callback) {
    callback(null); // No-op if no DurableStore
  }
};
 ...
 * 
 * @return {number} - Number of documents waiting to be written
 */
Connection.prototype.getWriteQueueSize = function() {
  return this.durableStore ? this.durableStore.getWriteQueueSize() : 0;
};

/**
 * Check if there are pending writes in the queue.
 * 
 * @return {boolean} - True if there are documents waiting to be written
 */
Connection.prototype.hasPendingWrites = function() {
  return this.durableStore ? this.durableStore.hasDocsInWriteQueue() : false;
};
...
Connection.prototype._destroyDoc = function(doc) {
  ...
  doc.emit('destroy');
  // TODO: remove from durable store?
};
