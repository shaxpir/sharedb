...
MessageBroker.prototype._initializeChannel = function() {
  ...
  if (typeof BroadcastChannel === 'undefined') {
    ...
    this._initTimer = setTimeout(function() {
      broker._initTimer = null;
      broker.emit('error', new Error('BroadcastChannel not supported'));
    }, 0);
    return;
  }
  ...
  try {
    ...
    // Emit ready asynchronously so listeners can be attached after construction
    this._initTimer = setTimeout(function() {
      broker._initTimer = null;
      broker.emit('ready');
    }, 0);
