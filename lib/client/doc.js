...
module.exports = Doc;
function Doc(connection, durableStore, collection, id) {
  ...
  // Internal flag to determine if the doc needs persistence. This helps us
  // prevent unnecessary duplicate writes to the DurableStore
  this._needsToPersist = false;
}
emitter.mixin(Doc);
...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  var doc = this;
  this.emit('enqueue persist', {reason: "ingestSnapshot"} );
  this._putToDurableStore(function() {
    doc.emit('load');
    callback && callback();
  });
};
...
Doc.prototype._handleOp = function(err, message) {
  ...
  try {
    ...
    this.emit('enqueue persist', {reason: "_handleOp, remote op"} );
    this._putToDurableStore();
  } catch (error) {
...
Doc.prototype._onConnectionStateChanged = function() {
  if (this.connection.canSend) {
    ...
    if (this.inflightOp) {
      ...
      this.inflightOp = null;
      this.emit('enqueue persist', {reason: "_onConnectionStateChanged", canSend: false, hadInflightOp: true} );
      this._putToDurableStore();
    }
    this.subscribed = false;
...
Doc.prototype._sendOp = function() {
  ...
  var doc = this;
  this._needsToPersist = false;
  this.emit('enqueue persist', {reason: '_sendOp'} );
  this._putToDurableStore(function() {
    doc.connection.sendOp(doc, op);
...
Doc.prototype._submit = function(op, source, callback) {
  ...
  // Flushing should either send the op, which will call _putToDurableStore()
  // in its own way, or, if the op wasn't sent for whatever reason (i.e.
  // connection is offline, or we are paused, or we already have an inflightOp)
  // then the op will remain in pendingOps and we still need to persist... We
  // set a flag to check if a persist occured in flush, and if not, persist
  // here. This is to prevent unnecessary duplicate persist calls
  doc._needsToPersist = true;
  util.nextTick(function() {
    doc.flush();
    if (doc._needsToPersist) {
      doc.emit('enqueue persist', {reason: "_submit, flush didn't persist", inflightOp: doc.inflightOp, _needsToPersist: true});
      doc._putToDurableStore();
      doc._needsToPersist = false;
    }
  });
};
...
Doc.prototype._clearInflightOp = function(err) {
  ...
  // Persist if flushing doesn't do it for us...
  this._needsToPersist = true;
  this.flush();
  if (this._needsToPersist) {
    this.emit('enqueue persist', {reason: "_clearInflightOp, flush didn't persist", _needsToPersist: true});
    this._putToDurableStore();
    this._needsToPersist = false;
  }
...
// Interacting with the DurableStore

Doc.prototype.ensureDocHasData = function(callback) {
  var doc = this;
  var callbackAndFetch = function() { callback && callback(); doc.fetch(); };
  // Ensure this data is loaded into memory, and call a remote fetch to keep the data fresh.
  // If there's already an in-memory object, just execute the callback-and-fectch.
  if (typeof(this.data) !== 'undefined') {
    return callbackAndFetch();
  }
  // The data is NOT already in memory. If the data is in the DurableStore, restore it to memory,
  // executing the callback-and-fetch once the durable-record has been restored.
  if (this.durableStore && this.durableStore.isDocInInventory(this.collection, this.id)) {
    this.durableStore.restoreDocFromDurableRecord(this, callbackAndFetch);
  }
  // If we couldn't find the content in memory or in the DurableStore, then pass the original callback
  // into a (non-background) fetch, which will be executed once we get the remote response.
  this.fetch(callback);
};

Doc.prototype.ensureDocHasRecentData = function(minVersion, callback) {
  var doc = this;
  var callbackAndFetch = function() { callback && callback(); doc.fetch(); };
  // Ensure this data is loaded into memory, and call a remote fetch to keep the data fresh. If there's
  // already an in-memory object (whose version satisfies the 'minVersion' parameter), just just execute
  // the callback-and-fectch.
  if (typeof(this.data) !== 'undefined' && typeof(this.durableStore.extVersionDecoder) != 'undefined') {
    // If this DurableStore has an `extVersionDecoder` function, then use it to decode the external
    // (e.g., app-specific) version identifier.
    var inMemoryDataVersion = this.version;
    if (typeof(this.durableStore.extVersionDecoder) !== 'undefined') {
      inMemoryDataVersion = this.durableStore.extVersionDecoder(this.data);
    }
    // Determine whether the in-memory version identifier satisfies the 'minVersion' parameter.
    if (this.durableStore.hasMinVersion(minVersion, inMemoryDataVersion)) {
      return callbackAndFetch();
    }
  }
  // The data is NOT already in memory (or the in-memory version doesn't satisfy the 'minVersion'
  // parameter). If the data is in the DurableStore, restore it to memory, executing the
  // callback-and-fetch once the durable-record has been restored.
  if (this.durableStore && this.durableStore.isDocInInventory(this.collection, this.id, minVersion)) {
    this.durableStore.restoreDocFromDurableRecord(this, callbackAndFetch);
  }
  // If we couldn't find the content in memory or in the DurableStore, then pass the original callback
  // into a (non-background) fetch, which will be executed once we get the remote response.
...
Doc.prototype._putToDurableStore = function(callback) {
  if (this.durableStore) {
    this.durableStore.putDoc(this, callback);
  } else {
    callback && callback();
  }
};
