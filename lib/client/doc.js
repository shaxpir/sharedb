...
Doc.prototype._setType = function(newType) {
  console.log('[Doc._setType] Called with:', newType, 'type:', typeof newType);
  
  if (typeof newType === 'string') {
    console.log('[Doc._setType] Looking up type in types.map for:', newType);
    console.log('[Doc._setType] Available types in map:', Object.keys(types.map));
    newType = types.map[newType];
    console.log('[Doc._setType] After lookup, newType is:', newType ? 'found' : 'not found');
  }
  ...
  if (newType) {
    ...
    console.log('[Doc._setType] ERROR: Type not recognized:', newType);
...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  if (!snapshot) return callback && setTimeout(callback);
  ...
  if (this.version > snapshot.v) return callback && setTimeout(callback);
...
Doc.prototype.ensureDocHasData = function(callback) {
  if (this.durableStore) {
    // Skip reading from the DurableStore if the in-memory data already exists
    if (typeof(this.data) !== 'undefined') {
      return callback && setTimeout(callback);
    }
    // If the doc is in the local inventory, then we can satisfy the fetch from the
    // DurableStore. Otherwise, we must either send the fetch to the server, even if
    // we have to wait to reconnect.
    var isDocInInventory = this.durableStore.isDocInInventory(this.collection, this.id);
    if (isDocInInventory) {
      this.durableStore.restoreDocFromDurableRecord(this, callback);
      return;
    }
  }
  // If there is no durable store, or if we couldn't find the minVersion of the
  // content in memory or in the local store, then fetch the content remotely.
  this.fetch(callback);
};
...
Doc.prototype.ensureDocHasRecentData = function(minVersion, callback) {
  if (this.durableStore) {
    // Skip reading from the DurableStore if the in-memory data already exists
    // and already satisfies the 'minVersion' constraint.
    if (typeof(this.data) !== 'undefined') {
    ...
    // DurableStore. Otherwise, we must either send the fetch to the server, even if
    // we have to wait to reconnect.
    var isDocInInventory = this.durableStore.isDocInInventory(this.collection, this.id, minVersion);
    if (isDocInInventory) {
      this.durableStore.restoreDocFromDurableRecord(this, callback);
      return;
    }
