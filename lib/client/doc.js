...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  var doc = this;
  // Skip persisting snapshots for non-existent documents (type is null).
  // A missing row already communicates "doesn't exist." Writing an empty row
  // creates a race: doc.create() defers its persist to nextTick, so readers
  // can briefly see a row with null data fields.
  if (!this.type) {
    doc.emit('load');
    return callback && setTimeout(callback);
  }
