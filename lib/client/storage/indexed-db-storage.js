 ...
 *
 * All callbacks follow error-first convention: callback(error, result)
 *
 ...
 */
IndexedDbStorage.prototype.initialize = function(callback) {
  var storage = this;
    ...
    if (storage.isBrandNew) {
      ...
      storage.writeRecords(
        ...
        function(err) {
          ...
          callback && callback(null, newInventory);
        }
      );
    } else {
      storage.ready = true;
  ...
  request.onerror = function(event) {
    var err = new Error('Error opening IndexDB (' + storage.dbName + '): ' + event.target.error);
    storage.debug && logger.error(err.message);
...
IndexedDbStorage.prototype.writeRecords = function(recordsByStoreName, callback) {
  ...
  transaction.onerror = function(event) {
    var duration = window.performance.now() - start;
    var err = new Error('Transaction error (' + itemCount + ' items) in ' + duration + ' millis: ' + event.target.error);
    storage.logError(err.message);
    callback && callback(err);
  };
};
...
IndexedDbStorage.prototype.readRecord = function(storeName, recordId, callback) {
  ...
  request.onerror = function(event) {
    var err = new Error('Error reading record (' + recordId + ') from store (' + storeName + '): ' + event.target.error);
    storage.logError(err.message);
    callback(err, null);
  };
};
...
IndexedDbStorage.prototype.deleteRecord = function(storeName, recordId, callback) {
  ...
  request.onerror = function(event) {
    var err = new Error('Error deleting record (' + recordId + ') from store (' + storeName + '): ' + event.target.error);
    storage.logError(err.message);
    callback && callback(err);
  };
};
...
IndexedDbStorage.prototype.readAllRecords = function(storeName, callback) {
  ...
  request.onsuccess = function(event) {
    ...
    callback(null, decryptedRecords);
  };

  request.onerror = function(event) {
    var err = new Error('Error reading all records from store (' + storeName + '): ' + event.target.error);
    storage.logError(err.message);
    callback(err, null);
  };
};
...
IndexedDbStorage.prototype.readRecordsBulk = function(storeName, ids, callback) {
  ...
  var processRecord = function(id) {
    ...
    request.onerror = function(event) {
      if (hasError) return;
      hasError = true;
      var err = new Error('Error bulk reading record (' + id + ') from store (' + storeName + '): ' + event.target.error);
      storage.logError(err.message);
      callback(err, null);
    };
  };
...
IndexedDbStorage.prototype.clearStore = function(storeName, callback) {
  ...
  request.onerror = function(event) {
    var err = new Error('Error clearing store (' + storeName + '): ' + event.target.error);
    storage.logError(err.message);
    callback && callback(err);
  };
};
...
IndexedDbStorage.prototype.clearAll = function(callback) {
  ...
  transaction.onerror = function(event) {
    var err = new Error('Error clearing all data: ' + event.target.error);
    storage.logError(err.message);
    callback && callback(err);
  };
};
