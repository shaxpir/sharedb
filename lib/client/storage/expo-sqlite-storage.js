...
/**
 * Validates that a store name is safe to use in SQL queries.
 * Only allows 'docs' and 'meta' as valid store names.
 */
ExpoSqliteStorage.prototype.validateStoreName = function(storeName) {
  if (storeName !== 'docs' && storeName !== 'meta') {
    throw new Error('Invalid store name: ' + storeName + '. Must be "docs" or "meta"');
  }
  return storeName;
};
...
ExpoSqliteStorage.prototype.writeRecords = function(recordsByStoreName, callback) {
  ...
  for (var i = 0; i < storeNames.length; i++) {
    ...
    var safeStoreName = storage.validateStoreName(storeName);
    db.runAsync(
      'INSERT OR REPLACE INTO ' + safeStoreName + ' (id, data) VALUES (?, ?)',
      insertions
    ).then(
      function() {
        var duration = Date.now() - start;
...
ExpoSqliteStorage.prototype.readRecord = function(storeName, recordId, callback) {
  ...
  try {
    ...
    storage.db.getFirstAsync(
      'SELECT data FROM ' + safeStoreName + ' WHERE id = ?',
      [recordId]
    ).then(
      function(record) {
        var duration = Date.now() - start;
...
ExpoSqliteStorage.prototype.deleteRecord = function(storeName, recordId, callback) {
  ...
  // Validate store name to prevent SQL injection
  var safeStoreName = storage.validateStoreName(storeName);
  storage.db.runAsync('DELETE FROM ' + safeStoreName + ' WHERE id = ?', [recordId]).then(function() {
    var duration = Date.now() - start;
