...
DurableStore.prototype.restoreDocFromDurableRecord = function(doc, callback) {
  var durableStore = this;
  console.log('[DurableStore] restoreDocFromDurableRecord called for', doc.collection + '/' + doc.id);
  
  this.getDoc(doc.collection, doc.id, function(durableRecord) {
    console.log('[DurableStore] getDoc returned:', durableRecord ? 'record found' : 'null record');
    
    // If no durable record exists, the document hasn't been cached yet
    if (!durableRecord) {
      console.log('[DurableStore] No durable record found, skipping restoration');
      callback && callback();
      return;
    }

    console.log('[DurableStore] Durable record structure:', {
      has_collection: durableRecord.hasOwnProperty('collection'),
      has_id: durableRecord.hasOwnProperty('id'),
      has_version: durableRecord.hasOwnProperty('version'),
      has_data: durableRecord.hasOwnProperty('data'),
      has_type_name: durableRecord.hasOwnProperty('type_name'),
      type_name_value: durableRecord.type_name,
      has_pendingOps: durableRecord.hasOwnProperty('pendingOps'),
      pendingOps_type: typeof durableRecord.pendingOps,
      pendingOps_isArray: Array.isArray(durableRecord.pendingOps),
      has_inflightOp: durableRecord.hasOwnProperty('inflightOp')
    });
    ...
    console.log('[DurableStore] About to call _setType with:', durableRecord.type_name);
    ...
    console.log('[DurableStore] Setting pendingOps:', durableRecord.pendingOps);
    doc.pendingOps = durableRecord.pendingOps;
    console.log('[DurableStore] doc.pendingOps after assignment:', doc.pendingOps, 'length:', doc.pendingOps ? doc.pendingOps.length : 'N/A');
    ...
    if (inflightOp) {
      console.log('[DurableStore] Found inflightOp, adding to pendingOps');
    ...
    doc.emit('restore');
    console.log('[DurableStore] Document restored successfully');
