 ...
 *
 * options.storage (string: must be 'indexed-db' or 'expo-sqlite'): Indicates the storage engine
 * used for persistence of the durable store records. The 'indexed-db' storage engine can be used
 * in a browser or on the desktop (with NWJS, Electron, etc). The 'sqlite-expo' can be used in
 * React Native with the 'expo' platform.
 *
...
function DurableStore(options) {
  ...
  if (options && options.storage && typeof (storageEngine) == 'string') {
    var storageEngine = options.storage.toLowerCase();
    if (storageEngine == 'indexed-db') {
      var IndexedDbStorage = require('./storage/indexed-db-storage');
      this.storage = new IndexedDbStorage(options);
    } else if (storageEngine == 'expo-sqlite') {
      var ExpoSqliteStorage = require('./storage/expo-sqlite-storage');
      this.storage = new ExpoSqliteStorage(options);
    } else {
      throw new Error('unknown storage engine for DurableStore: ' + storageEngine);
    }
  } else {
    throw new Error('no storage engine specified for DurableStore');
  }
